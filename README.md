...
**Update (deep-dive completed)**: all Frankfurt/Düsseldorf candidates got fully pulled, not just scoped. **Messe Frankfurt** shares one exhibitor-service API across its entire portfolio (public apikey baked into the site's JS bundle) — **Light + Building** (1,884 exhibitors), **ISH** (2,127, India 31), **Heimtextil** (1,182 — **India is the #1 exhibiting country, 304 of 1,182 = 25.7%**), **Ambiente** (3,655 — **India is #1 again, 515 = 14.1%**, ahead of China 372 and Germany 367 — the largest fair by exhibitor count where India tops the list), **Techtextil** (1,474, India 63, #6). A bonus cross-show pull (omitting the event filter) returned **3,114 India-HQ'd exhibitors across 40+ Messe Frankfurt shows worldwide** in one API call. **Messe Düsseldorf** shares a DIMEDIS "vis" A-Z directory API (confirmed via a cross-domain test: interpack.com's server + a header pointing at k-online.com returned K Fair's own data) — **interpack** (3,085, India 111), **EuroShop** (2,014, India only 8), **wire & Tube Düsseldorf** (2,743 combined, India 167, #5), **drupa** (1,725, India 70, #4 — the vis directory serves live year-round despite the next physical edition being 2028), **A+A** (2,514, India 61), **glasstec** (1,158, India 13), **boot** (1,773, India 1 — essentially no signal), **Caravan Salon** (1,008, India **0** — a confirmed, real negative, not a gap). Two domains turned out to be `.com` not the commonly-guessed `.de` (A+A, Caravan Salon) — caught by live-checking rather than assuming. Registry now tracks **64 events** total. Data: `data/trade_fairs/messe_frankfurt/`, `data/trade_fairs/messe_duesseldorf/`.
...
**India-domestic events (Delhi/Mumbai), deep-dived 2026-08-22** — six candidates from the earlier gap-scan, worked via 8 parallel subagents (adopted the [unlazy](https://github.com/Leonxlnx/unlazy) anti-laziness skill's Depth Tree approach: decompose into independent leaves, no agent allowed to declare "done" without a live source). Verified no exfiltration or hidden network calls before use — the flagged security-scanner risk was just its `spawnSync` gate-check surface. Results, in order of how much real data survived:
- **Convergence India** (Bharat Mandapam, 33rd edition, Mar 2026) — a genuine, complete, structured directory with a real per-exhibitor Country field, driven live across 43 pages: **1,021 exhibitors, 149 foreign (14.6%)** across 22 countries — China 46, Russia 24 (Moscow government pavilion), Taiwan 24 (TAITRA pavilion), US 15.
- **IIJS Bharat — Premiere** (GJEPC, 42nd edition, Aug 2026) — full 2,163-row roster served server-side in plain HTML, no gate: **76 foreign (3.5%)** — Italy 24, Thailand 13, Hong Kong 11, UAE 10.
- **IITF** (ITPO, 44th edition, Nov 2025) — two-layer finding: **11 countries** confirmed via a PIB press release (China, Thailand, UAE, Malaysia, Sweden, Turkey, Iran, South Korea, Egypt, Lebanon, Tunisia), plus **24 company-level rows** from ITPO's own official exhibitor PDF (Korea 10, Iran 3). Honest gap: Malaysia is named at country level but has zero matching company rows.
- **Cement Expo** (ASAPP Info Global, 14th edition, Dec 2023) — no live directory exists; recovered a real booth-numbered list from an archived brochure PDF's "GLIMPSES OF THE PAST" section: **63 companies, 19 foreign/foreign-parent** (Pentol Germany, MIDES Industriais Brazil directly; AMETEK, Testo, IKA, ABB, Horiba, Loesche, Evonik, Flender via verified India-subsidiary parents).
- **India Mobile Congress** (DoT + COAI, 9th edition, Oct 2025) — the organizer's own directory is a lead-gen form gate, correctly not filled in or bypassed; delivered a labeled lower-confidence fallback instead — 9 named companies from verifiable press/organizer sources (Nokia, Ericsson, Qualcomm, Quectel, Skylo, Samsung via its India subsidiary).
- **ACETECH** (ABEC Exhibitions, 19th Mumbai edition, Nov 2025) — genuinely no public roster anywhere checked (18-page sitemap, 10times.com, ExpoFP, Scribd); verified foreign count is a real **0**. Also caught and correctly *rejected* a false-positive foreign-company candidate mid-research (India-HQ'd firm with an old Italian JV, not foreign-HQ'd) — kept in the output as a worked example of avoiding name-similarity guessing. Two premise corrections surfaced along the way too: `acetechindia.com` is an unrelated software company, not the fair (`acetechexpo.com` is correct), and the organizer is ABEC, not "ThreeSixty Marketing Services" as originally assumed.

Data: `data/trade_fairs/india_domestic/{convergence_india,iijs,iitf,cement_expo,india_mobile_congress,acetech}/`.

## Association/exhibitor leads cross-reference (layer 41)

`scripts/build_layer41_association_leads_crossref.py` — cross-references layer 39/40's rosters against the twin's existing leads (layer 16) and company DB (layer 32) using the same word-boundary strong/weak fragment matcher as the filing sweeps. Sources: 4 re-parsed session scratch files (Aero India 2019, SEMICON India 2025, AAHAR 2024 Hall 1, India ITME 2022), 8 hand-curated association rosters, the durable `data/trade_fairs/railways/` datasets (InnoTrans 3,082; IREE 27 clean foreign), all 13 Frankfurt/Düsseldorf fairs, and the 5 India-domestic company-level finds (Convergence India, IIJS, IITF, Cement Expo, India Mobile Congress — ACETECH excluded, its roster is a verified zero, nothing to cross-reference). **26,191 new companies flagged** across 25 sources, checked against 1,769 known names — up from 14,570 after the second Messe/India-domestic pass, driven mostly by the 7 newly-pulled Frankfurt/Düsseldorf fairs (Ambiente alone contributes 3,259). Caught and fixed a real bug along the way: the loader was reading `country_label` for every Messe file, but Düsseldorf's vis-platform CSVs use a plain `country` column — the Messe Frankfurt entries were fine, Düsseldorf ones were silently losing their country tag. Also fixed the original AAHAR parsing bug (see prior note below) and filtered out embassies/trade-promotion bodies (e.g. "High Commission of Canada") that were getting flagged as false-positive "new companies." 🔴 The 4 original scratch-file sources are session-ephemeral and won't survive to a future run — re-fetch via the URLs in layer 40 first; everything under `data/trade_fairs/` is durable repo data and will. Output: `layers/41_association_leads_crossref.json`, doc: `docs/ASSOCIATION_LEADS_CROSSREF.md`.
