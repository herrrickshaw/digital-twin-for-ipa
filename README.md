...
Other new-sector candidates identified but not yet deep-dived: **Light + Building** and **ISH** (Frankfurt — construction/building automation), **interpack** and **EuroShop** (Düsseldorf — packaging, retail tech), **wire & Tube Düsseldorf** (extends existing Steel & Metals coverage downstream), **Heimtextil** (Frankfurt — home/contract textiles, distinct from ITMA's machinery focus), **India International Jewellery Show** (Mumbai), **India Mobile Congress** (Delhi — telecom, distinct from Electronics & Semiconductors). Registry now tracks **52 events** total.

## Association/exhibitor leads cross-reference (layer 41)

`scripts/build_layer41_association_leads_crossref.py` — cross-references layer 39/40's rosters against the twin's existing leads (layer 16) and company DB (layer 32) using the same word-boundary strong/weak fragment matcher as the filing sweeps, re-parsing this session's raw scratch files (Aero India 2019 foreign exhibitors, SEMICON India 2025, AAHAR 2024 Hall 1, India ITME 2022 international) plus the hand-curated smaller association rosters (JAMA, ASD Europe, EFPIA, ICMM, IPIECA, Cefic, worldsteel sample, VDA Group I). **733 new companies flagged** across 12 sources, checked against 1,769 known names. Caught and fixed a real parsing bug along the way: the first AAHAR pass swallowed every subsequent exhibition hall (1,577 rows instead of the correct 139) because the stop-marker regex looked for "HALL NO. 2" when the actual text used "HALL 2 GROUND FLOOR" — caught via a sanity check against the scouting agent's own reported count. Also filtered out embassies/trade-promotion bodies (e.g. "High Commission of Canada") that were getting flagged as false-positive "new companies." 🔴 The raw scratch-file sources are session-ephemeral and won't survive to a future run — re-fetch via the URLs in layer 40 first. Output: `layers/41_association_leads_crossref.json`, doc: `docs/ASSOCIATION_LEADS_CROSSREF.md`.
