...
CBG_GOBARDHAN = {
    "what": ("CBG plants selling fertiliser — the FCO route that lets Compressed "
             "Bio-Gas / GOBARdhan plants sell their fermented digestate as a "
             "notified fertiliser, a second revenue leg for the CBG/biofuel "
             "business model (FOM is ~20 pct of a CBG plant's feedstock "
             "throughput). Researched 2026-07-24 against the primary gazette."),
    "regulator": ("Department of Agriculture & Farmers Welfare (DA&FW) under the "
                  "Fertiliser (Inorganic, Organic or Mixed) Control Order, 1985 "
                  "(FCO) — NOT the Ministry of Chemicals & Fertilizers"),
    "two_moves": {
        "1_create_category": ("Schedule VIII 'Organic Carbon Enhancer' introduced "
                              "17-Feb-2025; full FOM (solid) + LFOM (liquid) specs "
                              "notified 27-Mar-2025 (the 12th amendment to FCO 1985)"),
        "2_authorise_bulk_sale": ("under sub-clause (c) of clause 22 of the FCO, "
                                  "serial 'Bulk Sale Notifications' name specific "
                                  "plants and authorise them to sell FOM/LFOM in "
                                  "BULK directly to farmers for 3 years — bypassing "
                                  "the normal fertiliser-dealer licensing chain"),
    },
    "why_it_was_needed": ("CBG digestate/bio-slurry could not be sold as fertiliser "
                          "before: FOM kept failing the old 5 pct minimum NPK and "
                          "the 4mm sieve at 70 pct moisture. Schedule VIII reframes "
                          "it around SOIL CARBON — the new 'Organic Carbon Enhancer' "
                          "category DROPS NPK and C:N entirely and is defined on "
                          "organic-carbon + heavy-metals only. Critics (Down To "
                          "Earth/CSE) note this relaxes the organic-manure quality "
                          "norm."),
    "spec_confirmed_primary_gazette": ("CG-DL-E-29032025-262118 — 'Fertiliser "
                                       "(Inorganic, Organic or Mixed) (Control) "
                                       "(THIRD) Amendment Order, 2025', issued "
                                       "27-Mar-2025, publ 29-Mar-2025, Part II Sec "
                                       "3(ii), Schedule VIII Part A; pulled from "
                                       "egazette.gov.in/WriteReadData/2025/262118.pdf "
                                       "on 2026-07-24 (browser-driven ASP.NET search "
                                       "-> direct PDF URL). NOT the '12th amendment' "
                                       "that secondary sources called it — it is the "
                                       "3rd Amendment of 2025."),
    "fom_spec_schedule_viii": {   # PRIMARY GAZETTE — the actual Schedule VIII FOM spec
        "moisture_max_pct": "30-70",
        "organic_carbon_min_pct": "12-14",
        "pH": "6.0-8.4",
        "electrical_conductivity_max": "4.0 (dSm-1)",
        "pathogens": "Nil",
        "heavy_metals_mg_per_kg_max": {"As": 10, "Cd": 5, "Cr": 50, "Cu": 300,
                                       "Hg": 0.15, "Ni": 50, "Pb": 100, "Zn": 1000},
        "no_NPK_or_CN": ("Schedule VIII FOM has NO NPK minimum, NO C:N ratio, NO "
                         "sieve requirement — defined on carbon + heavy metals only"),
        "correction": ("🔴 SUPERSEDES the earlier FACT-tender numbers in this block "
                       "(which had NPK 1.2 pct / C:N <=30 / sieve / TOC flat-14 / "
                       "Cr 300 / Cu 50 / pH 6.5-8.4) — those were a DIFFERENT/older "
                       "FOM standard, not Schedule VIII. Gazette-correct: carbon "
                       "12-14 pct, pH 6.0-8.4, Cr 50 / Cu 300, no NPK/C:N/sieve"),
    },
    "lfom_spec_schedule_viii": {   # PRIMARY GAZETTE — confirmed
        "moisture_liquid_max_pct": "90-97",
        "organic_carbon_min_pct_by_weight": "1.5",
        "pH": "6.0-8.4",
        "electrical_conductivity_max": "4.0 (dSm-1)",
        "pathogens": "Nil (infectious)",
        "heavy_metals_mg_per_kg_max": {"As": 10, "Cd": 5, "Cr": 50, "Cu": 300,
                                       "Hg": 0.15, "Ni": 50, "Pb": 100, "Zn": 1000},
        "analysis_note": ("gazette Part C: LFOM parameters estimated on DRY-WEIGHT "
                          "basis — oven-dry 1000 ml at 105C before analysis"),
        "confirmation_status": ("✅ CONFIRMED against primary gazette "
                                "CG-DL-E-29032025-262118 (2026-07-24)"),
        "correction": ("🔴 SUPERSEDES the earlier GeM-2023 numbers (TOC 14 pct 'dry "
                       "basis', C:N <20, Pb 50). Gazette-correct LFOM: organic "
                       "carbon 1.5 pct BY WEIGHT (liquid), pH 6.0-8.4, same heavy "
                       "metals as FOM incl. Pb 100; NO NPK/C:N. The Cr/Cu 'transpose' "
                       "flag is RESOLVED — both FOM and LFOM are Cr 50 / Cu 300; the "
                       "old FACT-tender FOM table was the wrong one"),
        "amendment_naming": ("the operative instrument is the 3rd Amendment Order "
                             "2025 (27-Mar-2025); secondary sources mis-labelled it "
                             "the '12th amendment' and referenced a later 3-Dec-2025 "
                             "amendment on sampling/analysis — the SPEC numbers above "
                             "are from the 27-Mar-2025 primary gazette"),
    },
    "egazette_access": ("🔑 egazette.gov.in is a session-token ASP.NET site (curl-"
                        "hostile: SearchMenu.aspx 302s to error without the session "
                        "cookie). Route that WORKS: browser-drive the home page's "
                        "Search postback (__doPostBack('sgzt','')) -> 'Search by "
                        "Ministry' (btnMinistry) -> set ddlMinistry=5 (Agri & "
                        "Farmers Welfare) + ddlmonth + ddlyear -> ImgSubmitDetails; "
                        "each result's download button hits the DIRECT PDF at "
                        "egazette.gov.in/WriteReadData/<year>/<gid-numeric>.pdf "
                        "(no session needed for the PDF itself)"),
    ...
    "authorised_plants": {
        ...
        "V_2025_07_08": [
            ...
            "Reliance Chemicals & Materials Ltd — Hoshiarpur CBG (Punjab)",
            "Reliance Bioenergy Ltd — Jhajjar CBG (Haryana)",
            "Reliance New Solar Energy Ltd — Jamnagar CBG (Gujarat)",
            "Kota Development Authority (Rajasthan)",
            "Lakhimpur Kheri RNG Pvt Ltd (UP)",
            "Reliance Bioenergy Ltd — Nagpur CBG (Maharashtra)",
            "Reliance Bioenergy Ltd — Panipat CBG (Haryana)",
            "Patiala RNG Pvt Ltd (Punjab)",
            "Reliance Industries Ltd — Prayagraj CBG (UP)",
            "Sangrur RNG Pvt Ltd (Punjab)",
            "Transtech Punjab CBG LLP (Fatehabad, Haryana)",
            "Reliance Chemicals & Materials Ltd — Vadodara CBG (Gujarat)",
            "XEMX Projects Pvt Ltd (Maharashtra)",
        ],
        "VII_2026_02_25": [
            "Prayagraj RNG Pvt Ltd (UP)", "Kakadiya Green Energy Pvt Ltd (Bhavnagar, Gujarat)",
            "AnandMangal Infradevelopers Pvt Ltd (Meerut, UP)",
            "Shree Jee Bioenergy (Haridwar, Uttarakhand)",
            "Reliance Industries Ltd — Indore CBG (MP)",
            "Reliance Industries Ltd — Bhopal CBG (MP)",
            "Adani Total Energies Biomass Ltd (Mathura, UP)",
            "VINP Distilleries and Sugars Pvt Ltd (Haveri, Karnataka)",
            "Enlighten Biofuels and Bio Fertilizers Pvt Ltd (Najafgarh, Delhi)",
            "Carbon Masters India Pvt Ltd — Mattikere, Bengaluru",
            "Carbon Masters India Pvt Ltd — Kalidasa Rd, Bengaluru",
    ...
    "mda_economics": {
        ...
        "rate": "Rs 1,500 per metric tonne for FOM / LFOM / PROM from biogas/CBG plants",
        "allocation": "Rs 1,451.84 crore",
        "portal": "iFMS (Integrated Fertilizer Management System) registration",
        "plants_registered": "120 CBG/biogas plants on iFMS (as of 4-Mar-2026)",
        "sales_trajectory": ("56,058 MT (FY24) -> 3.36 lakh MT (FY25) -> 16.72 lakh "
                             "MT (FY26 to 28-Feb) — ~30x in two years"),
    },
    "gobardhan": ("Galvanizing Organic Bio-Agro Resources Dhan — the 'waste to "
                  "wealth' umbrella tying CBG energy to the fertiliser story; "
                  "circulars on the EIL-run portal gobardhan.eil.co.in/"
                  "important-circular; unified registration via Dept of Drinking "
                  "Water & Sanitation"),
    "db_linkage": ("connects to the twin's E20 ethanol / biofuel EC-filer cohort "
                   "(layer 32/33) — VINP Distilleries & Sugars is on the FOM "
                   "bulk-sale list AND is a distillery; the FCO fertiliser leg + "
                   "MDA subsidy materially improve CBG/biofuel project economics, "
                   "the flip side of the Lalta-Prasad state-fee RISK on the same "
                   "cohort"),
    "sources": ["gazette VII S.O.1018(E) 25-Feb-2026 (primary, clause 22(c) FCO)",
                "gobardhan.eil.co.in/important-circular (bulk-sale I-VII)",
                "FACT tender (Schedule VIII FOM spec table)",
                "IBEF/NewsOnAir (MDA Rs 1,500/MT, 120 plants), Down To Earth/CSE (critique)"],
}
...
def main():
    ...
    if live:
        for u in ("https://pmmsy.dof.gov.in/", "https://fidf.in/home/",
                  ...
                  "https://gobardhan.eil.co.in/important-circular"):
            checked[u] = status(u)
    ...
    layer = {
        ...
        "what": ("Fisheries incentive stack, state-excise bottling-fee economics, "
                 "import/export duty posture, India Exim Bank, and the CBG/"
                 "GOBARdhan FCO fertiliser route — analysis blocks with per-claim "
                 "sources, HTTP-verified access routes, and explicit layer-32 "
                 "company linkages. Research sweeps 2026-07-23/24."),
        "fisheries": FISHERIES,
        "excise_bottling": EXCISE_BOTTLING,
        "customs_duties": CUSTOMS_DUTIES,
        "exim_bank": EXIM_BANK,
        "cbg_gobardhan_fertiliser": CBG_GOBARDHAN,
